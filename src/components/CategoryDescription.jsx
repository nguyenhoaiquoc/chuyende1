...
export default function CategoryDescription({ description }) {
  ...
  return (
    <div className="mt-8 md:mt-12 pt-6 border-t relative"> {/* Thêm relative cho gradient */}
      <div
        ref={contentRef}
        // Thêm class `prose` (cần plugin @tailwindcss/typography)
        className={`prose prose-sm max-w-none transition-all duration-500 ease-in-out ${needsTruncation && !isExpanded ? 'max-h-[400px] overflow-hidden' : 'max-h-none'}`} 
