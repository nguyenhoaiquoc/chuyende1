...
export default function CartAdmin() {
    ...
    const fetchOrders = async () => {
      try {
        const response = await fetch(
          "https://ns414sbifk.execute-api.ap-southeast-1.amazonaws.com/api/cart"
        );
  ...
  const handleConfirmOrder = async (orderId) => {
    ...
    try {
      // 2. Gọi API PUT lên AWS
      const response = await fetch(
        `https://ns414sbifk.execute-api.ap-southeast-1.amazonaws.com/api/cart/${orderId}`,
        {
          method: "PUT",
          headers: {
      ...
      // 3. Nếu Server trả về OK thì mới cập nhật giao diện
      if (response.ok) {
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Xem thông tin khách hàng và sản phẩm đã đặt
          </p>
        </div>
        <Link
          to="/admin"
              ...
              <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2">
                  <h3 className="font-bold text-gray-900 uppercase text-xs tracking-wider border-b pb-2 mb-4">
                    Sản phẩm đã mua ({order.products?.length || 0})
                  </h3>
